...
describe('UndoEngine reverse intents', () => {
  ...
  it('reverses add_tag with remove_tag', () => {
    const runtime = getOperationRuntime();
    loadNode(runtime, { blockId: 'a' });
    const engine = getUndoEngine();
  ...
  it('reverses update_node with original field values', () => {
    const runtime = getOperationRuntime();
  ...
  it('reverses move_node with original parent and position', () => {
    const runtime = getOperationRuntime();
