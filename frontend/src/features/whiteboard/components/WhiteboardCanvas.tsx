...
function ShortcutRow({ keys, action }: { keys: string; action: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--spacing-1) 0', borderBottom: 'var(--border-width-default) solid var(--color-outline-variant)' }}>
      ...
      <kbd style={{ background: 'var(--color-surface-variant)', padding: 'var(--spacing-micro) var(--spacing-2)', borderRadius: 'var(--shape-small)', fontSize: 'var(--font-size-sm)', fontFamily: 'var(--font-family-mono)', color: 'var(--color-on-surface-variant)' }}>{keys}</kbd>
    </div>
  );
}
...
import { useGridSettings } from '@/features/whiteboard/hooks/useWhiteboardSelectors';
...
export const WhiteboardCanvas: React.FC<WhiteboardCanvasProps> = ({
  ...
  const { viewport } = data;
  const { gridSnap } = useGridSettings();
      ...
      >
        ...
        {el.type === 'text' && (
          ...
          >
            {editingTextId === el.id ? (
              <textarea
                aria-label="Edit whiteboard text"
  ...
  return (
    ...
    >
      ...
      {/* Keyboard shortcuts modal */}
      {showShortcuts && (
        // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-noninteractive-element-interactions -- modal backdrop closes on click
        <div
          ref={shortcutsRef}
          className="wb-shortcuts-modal"
          role="dialog"
          aria-modal="true"
          aria-label="Keyboard shortcuts"
          style={{ position: 'absolute', inset: 0, zIndex: 'var(--wb-z-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-scrim)' }}
          onClick={(e) => { if (e.target === e.currentTarget) setShowShortcuts(false); }}
        >
          <div className="wb-shortcuts-modal__content" style={{ background: 'var(--color-surface)', borderRadius: 'var(--shape-large)', padding: 'var(--spacing-6)', maxWidth: 'var(--whiteboard-shortcuts-max-width)', width: '90%' }}>
