...
import { $isInlineLinkNode } from '@/features/editor/editor/nodes/InlineLinkNode';
...
export function CustomCaretPlugin({ readOnly = false }: { readOnly?: boolean }): JSX.Element | null {
  ...
  const updateActiveBlock = useCallback(() => {
    ...
    if (newBlock !== activeBlockRef.current) {
      ...
      activeBlockRef.current?.querySelectorAll('.bullet-wrapper').forEach((bullet) => {
        bullet.removeAttribute('data-editing');
      });
      newBlock?.classList.add('node-block--editing');
      newBlock?.querySelectorAll('.bullet-wrapper').forEach((bullet) => {
        bullet.setAttribute('data-editing', 'true');
      });
  ...
  const updateCaretPosition = useCallback(() => {
    ...
    if (overwriteMode) {
      ...
      caret.style.width = 'var(--border-width-thick)';
