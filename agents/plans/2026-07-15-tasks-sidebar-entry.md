...
- Spec: `agents/superpowers/specs/2026-07-15-tasks-sidebar-entry-design.md`.
...
- Modify: `agents/plans/plan-totp-2fa.md:88-89`
- Modify: `docs/SECURITY.md:34`

**Interfaces:**
- Consumes: nothing.
- Produces: docs that describe the Flutter native app instead of the Kotlin WebView wrapper. `agents/mobile-sync.md` (sqflite/FTS5 notes about the Flutter app) and `agents/build-and-release.md` (already points at `notees-flutter`) are verified current and need NO changes.
...
In `agents/plans/plan-totp-2fa.md` replace:
