...
class TestEmitOp:
    async def test_emit_op_creates_plugin_op_log_row(self, plugin_context: PluginContext) -> None:
        ...
        await store.sync()
        rows = await store.query("SELECT plugin_id, op_type, data_json FROM plugin_op_log")
        assert len(rows) == 1
    ...
    async def test_emit_op_with_node_id_includes_node_id(self, plugin_context: PluginContext) -> None:
        await plugin_context.emit_op(
            workspace_uuid="ws-1",
            actor_uuid="actor-1",
            op_type="test.op",
        ...
        store = await plugin_context._get_workspace_store("ws-1", "actor-1")
        await store.sync()
        rows = await store.query("SELECT plugin_id, op_type, node_id, data_json FROM plugin_op_log")
        assert len(rows) == 1
...
class TestCreatePage:
    async def test_create_page_emits_node_and_properties(self, plugin_context: PluginContext) -> None:
        ...
        prop_rows = await store.query("SELECT property_schema_id, value FROM property_value")
...
class TestFindPageByName:
    async def test_find_page_by_name_returns_matching_uuid(self, plugin_context: PluginContext) -> None:
        node_uuid = await plugin_context.create_page(
            workspace_uuid="ws-1",
            actor_uuid="actor-1",
            name="Target Page",
    ...
    async def test_find_page_by_name_returns_none_when_missing(self, plugin_context: PluginContext) -> None:
        result = await plugin_context.find_page_by_name("ws-1", "actor-1", "Missing")
        assert result is None


class TestUpsertPageByExternalId:
    async def test_upsert_creates_new_page(self, plugin_context: PluginContext) -> None:
    ...
    async def test_upsert_updates_existing_page(self, plugin_context: PluginContext) -> None:
        schema_uuid = "ext-schema-1"
        node_uuid = await plugin_context.upsert_page_by_external_id(
            workspace_uuid="ws-1",
            actor_uuid="actor-1",
        ...
        store = await plugin_context._get_workspace_store("ws-1", "actor-1")
        await store.sync()
        rows = await store.query("SELECT id, content FROM node WHERE id = ?", (node_uuid,))
        assert len(rows) == 1
...
class TestEnsureClass:
    async def test_ensure_class_returns_none_for_page_kind(self, plugin_context: PluginContext) -> None:
    ...
    async def test_ensure_class_creates_new_class_for_unknown_name(self, plugin_context: PluginContext) -> None:
        class_uuid = await plugin_context.ensure_class("ws-1", "actor-1", "custom")

        store = await plugin_context._get_workspace_store("ws-1", "actor-1")
        await store.sync()
        rows = await store.query("SELECT id, name FROM class WHERE id = ?", (class_uuid,))
        assert len(rows) == 1
        assert rows[0]["name"] == "custom"
        await store.close()

    async def test_ensure_class_resolves_system_name_without_emitting(self, plugin_context: PluginContext) -> None:
        ...
        again = await plugin_context.ensure_class("ws-1", "actor-1", "Source", icon="book-open-variant")
    ...
    async def test_ensure_class_converges_on_repeated_calls(self, plugin_context: PluginContext) -> None:
    ...
    async def test_ensure_class_finds_preexisting_class_by_name(self, plugin_context: PluginContext) -> None:
...
class TestEnsurePropertySchema:
    async def test_ensure_property_schema_emits_operation(self, plugin_context: PluginContext) -> None:
        schema_uuid = await plugin_context.ensure_property_schema("ws-1", "actor-1", "external_id", prop_type="text")
    ...
    async def test_ensure_property_schema_resolves_system_name_without_emitting(
        ...
        result = await plugin_context.ensure_property_schema("ws-1", "actor-1", "citekey")
    ...
    async def test_ensure_property_schema_converges_on_repeated_calls(self, plugin_context: PluginContext) -> None:
        first = await plugin_context.ensure_property_schema("ws-1", "actor-1", "Zotero Key", icon="identifier")
        for _ in range(3):
            again = await plugin_context.ensure_property_schema("ws-1", "actor-1", "Zotero Key", icon="identifier")
            assert again == first
    ...
    async def test_ensure_property_schema_passes_through_schema_attributes(self, plugin_context: PluginContext) -> None:
        ...
        await store.sync()
        rows = await store.query(
            "SELECT type, multi, is_system, scope, class_filter_uuids FROM property_schema WHERE id = ?",
            (schema_uuid,),
        )
        assert len(rows) == 1
        row = rows[0]
...
class TestSetClassExtends:
    async def test_set_class_extends_sets_parents(self, plugin_context: PluginContext) -> None:
        ...
        store = await plugin_context._get_workspace_store("ws-1", "actor-1")
        await store.sync()
        rows = await store.query("SELECT extends_class_ids FROM class WHERE id = ?", (child,))
        assert len(rows) == 1
        assert json.loads(rows[0]["extends_class_ids"]) == [parent]
        await store.close()

    async def test_set_class_extends_rejects_cycles(self, plugin_context: PluginContext) -> None:
        ...
        with pytest.raises(ValueError, match="cycle"):
            await plugin_context.set_class_extends("ws-1", "actor-1", class_b, [class_a])


class TestFindOrCreateNodeByName:
    async def test_returns_same_node_on_repeated_calls(self, plugin_context: PluginContext) -> None:
        agent = SYSTEM_CLASS_UUIDS["agent"]
        first = await plugin_context.find_or_create_node_by_name("ws-1", "actor-1", agent, "Frank Herbert")
        second = await plugin_context.find_or_create_node_by_name("ws-1", "actor-1", agent, "Frank Herbert")
        assert first == second
    ...
    async def test_same_name_with_different_class_creates_new_node(self, plugin_context: PluginContext) -> None:
        agent = SYSTEM_CLASS_UUIDS["agent"]
        other = await plugin_context.ensure_class("ws-1", "actor-1", "OtherCls")
        first = await plugin_context.find_or_create_node_by_name("ws-1", "actor-1", agent, "Frank Herbert")
        second = await plugin_context.find_or_create_node_by_name("ws-1", "actor-1", other, "Frank Herbert")
        assert first != second

    async def test_creates_node_with_property_values(self, plugin_context: PluginContext) -> None:
        agent = SYSTEM_CLASS_UUIDS["agent"]
        schema_uuid = await plugin_context.ensure_property_schema("ws-1", "actor-1", "External Ref")
...
class TestZoteroStyleDoubleProvisioning:
    async def test_two_syncs_converge_without_duplicates(self, plugin_context: PluginContext) -> None:
        """Two Zotero-style syncs produce one schema, no ad-hoc class, one agent."""

        async def _sync_once() -> tuple[str | None, str, str]:
            source_class_uuid = await plugin_context.ensure_class("ws-1", "actor-1", "Source", icon="book-open-variant")
