...
async def _create_task_node(client, name: str = "Task") -> dict:
    ...
    r = await client.post(
        "/api/nodes/",
        json={"name": name, "is_page": True, "class_uuids": [task_class_uuid]},
    )
    assert r.status_code == 200, r.text
    return r.json()
...
async def test_recurrence_crud(authenticated_client, test_user):
    ...
    # Initially no rule
    r = await authenticated_client.get(f"/api/tasks/{node_uuid}/recurrence")
    assert r.status_code == 200
    assert r.json() is None

    # Create a weekly rule
    r = await authenticated_client.put(
        f"/api/tasks/{node_uuid}/recurrence",
    ...
    # Delete
    r = await authenticated_client.delete(f"/api/tasks/{node_uuid}/recurrence")
    assert r.status_code == 200
...
async def test_completion_crud(authenticated_client, test_user):
    ...
    r = await authenticated_client.post(
        f"/api/tasks/{node_uuid}/completions",
...
async def test_recurrence_rejects_non_task(authenticated_client, test_user):
    r = await authenticated_client.post("/api/nodes/", json={"name": "Not a task", "is_page": True})
    assert r.status_code == 200, r.text
