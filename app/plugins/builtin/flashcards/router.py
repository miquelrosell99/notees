...
async def _to_response(card) -> FlashcardResponse:
    return FlashcardResponse(
        id=card.id,
        uuid=card.uuid,
        node_uuid=card.node_uuid,
...
async def get_due_cards(
    ...
    return ReviewQueueResponse(
        cards=[await _to_response(c) for c in cards],
        total_due=len(cards),
    )
...
async def create_flashcard(
    ...
    try:
        card = await service.create_flashcard(body.node_uuid, body.front_text, body.back_text)
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc
    return await _to_response(card)
...
async def review_flashcard(
    ...
    try:
        updated = await service.review_card(node_uuid, body.grade)
    except ValueError as exc:
