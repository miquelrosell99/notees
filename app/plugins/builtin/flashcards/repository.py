...
class PostgresFlashcardRepository(FlashcardRepository):
    ...
    async def create(
        ...
        async with get_connection() as conn:
            sql = """
                INSERT INTO flashcard (node_uuid, workspace_id, user_id, front_text, back_text)
                VALUES ($1, $2, $3, $4, $5)
                ...
                RETURNING *
            """
            row = await conn.fetchrow(sql, node_uuid, workspace_id, user_id, front_text, back_text)
            return self._row_to_data(dict(row))

    async def get_by_node_uuid(self, node_uuid: str) -> FlashcardData | None:
        async with get_connection() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM flashcard WHERE node_uuid = $1",
                node_uuid,
            )
            if not row:
                return None
    ...
    async def delete(self, node_uuid: str) -> None:
        async with get_connection() as conn:
            await conn.execute("DELETE FROM flashcard WHERE node_uuid = $1", node_uuid)
