...
class ClassExtensionService:
    ...
    async def get_all_extended_classes_batch(
        self, class_ids: list[int]
    ) -> dict[int, list[int]]:
        """Get all ancestor class IDs for multiple classes in a bounded number of queries.

        Returns a mapping of class_id -> list of inherited class IDs (excluding the
        class itself), preserving depth-first order and detecting circular references.
        """
        if not class_ids:
            return {}

        # Fetch the direct-extension graph for every class reachable from the
        # requested set, including ancestors that are not in the initial set.
        direct: dict[int, list[int]] = {}
        to_fetch = set(class_ids)
        while to_fetch:
            batch = list(to_fetch)
            fetched = await self._class_extend_repo.get_extended_classes_batch(batch)
            next_parents: set[int] = set()
            for cid in batch:
                direct.setdefault(cid, fetched.get(cid, []))
                for parent_id in direct[cid]:
                    if parent_id not in direct:
                        next_parents.add(parent_id)
            to_fetch = next_parents

        # Compute transitive closures with cycle detection.
        cache: dict[int, list[int]] = {}

        def dfs(cid: int, stack: list[int]) -> list[int]:
            if cid in cache:
                return cache[cid]
            if cid in stack:
                # Circular reference detected; stop traversing this branch.
                raise CircularInheritanceError(list(stack) + [cid])
            stack.append(cid)
            result: list[int] = []
            for parent_id in direct.get(cid, []):
                result.append(parent_id)
                with contextlib.suppress(CircularInheritanceError):
                    result.extend(dfs(parent_id, stack))
            stack.pop()
            cache[cid] = result
            return result

        return {cid: dfs(cid, []) for cid in class_ids}
