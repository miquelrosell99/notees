...
class NodeService:
    ...
    def __init__(
        ...
        view_repo: NodeViewRepository | None = None,
    ):
        ...
        self._asset_file_service = asset_file_service
    ...
    def workspace_uuid(self) -> str | None:
        """Workspace UUID for this service instance."""
    ...
    def permissions(self) -> PermissionChecker:
        if self._permissions is None:
            ...
            if self._permission_repo is None:
                raise RuntimeError("Permission repository required for permission checks")
            self._permissions = PermissionChecker(self._user_id, self._permission_repo)
    ...
    async def list_nodes(
        ...
        return await self._node_repo.list_nodes(
    ...
    async def get_recent_pages(self, limit: int) -> list[Node]:
        """Get recently opened pages."""
        return await self._node_repo.get_recent_pages(limit)

    async def get_random_pages(self, limit: int) -> list[Node]:
        """Get random non-deleted, non-system pages."""
        return await self._node_repo.get_random_pages(limit)

    async def get_recently_created_pages(self, limit: int) -> list[Node]:
        """Get recently created pages."""
        return await self._node_repo.get_recently_created_pages(limit)

    async def get_workspace_data(self, page: int, page_size: int) -> tuple[int, list[dict[str, Any]], list[dict[str, Any]]]:
        """Return workspace visualization data: (total, nodes, links)."""
        return await self._node_repo.get_workspace_data(page, page_size)

    async def get_workspace_nodes(self, page: int, page_size: int) -> tuple[int, list[dict[str, Any]]]:
        """Return workspace nodes without links: (total, nodes)."""
        return await self._node_repo.get_workspace_nodes(page, page_size)
    ...
    async def get_links_for_nodes(
        ...
        return await self._link_service.get_links_for_nodes(
    ...
    async def search_by_uuid_prefix(self, uuid_prefix: str, limit: int) -> list[Node]:
        ...
        return await self._node_repo.search_by_uuid_prefix(uuid_prefix, limit)
    ...
    async def expand_class_hierarchy(self, class_ids: list[int]) -> set[int]:
        ...
        return await self._class_service.expand_class_hierarchy(class_ids)
    ...
    async def get_ancestors_batch(
        ...
        return await self._node_repo.get_ancestors_batch(node_ids, include_self)
    ...
    async def get_page_id_by_uuid(self, uuid: str) -> int | None:
        """Get the internal ID of an active page node by UUID."""
        return await self._node_repo.get_page_id_by_uuid(uuid)

    async def list_daily_pages_paginated(self, page: int, page_size: int) -> tuple[int, list[Any]]:
        """List daily pages ordered by UUID desc."""
        return await self._node_repo.list_daily_pages_paginated(page, page_size)
    ...
    async def get_alias_node_ids(self, target_node_id: int) -> list[int]:
        ...
        return await self._link_service.get_alias_node_ids(target_node_id)
    ...
    async def get_alias_node_ids_batch(self, target_node_ids: list[int]) -> dict[int, list[int]]:
        ...
        return await self._link_service.get_alias_node_ids_batch(target_node_ids)
    ...
    async def get_extended_classes_batch(self, node_ids: list[int]) -> dict[int, list[int]]:
        ...
        return await self._class_service.get_extended_classes_batch(node_ids)
    ...
    async def get_effective_class_ids_batch(self, node_ids: list[int]) -> dict[int, list[int]]:
        ...
        extension_service = self._class_service.get_class_extension_service(
        ...
        # Batch-resolve the full transitive closure for every explicit class in
        # a constant number of queries instead of one per class.
        extends_cache = await extension_service.get_all_extended_classes_batch(
            list(all_explicit_class_ids)
        )
    ...
    async def update_node(
        ...
        Optionally reconciles classes and applies property values in the same
        transaction.
        ...
        Args:
            ...
            classes: If given, set the node's classes to exactly this list
            properties: Optional {property_id: value} map to apply
        """
        # Strip trailing spaces from name
        if data.name is not None:
            data.name = data.name.rstrip()
        ...
        if old_node.is_page and (data.name is not None or data.parent_id is not None):
            ...
            if data.name is not None or data.parent_id is not None:
                ...
                check_classes = await self._link_service.get_inline_class_targets(node_id)
    ...
    async def apply_node_extras(
        ...
        if classes is not None:
            current = set(await self.get_node_class_ids(node_id))
            want = set(classes)
    ...
    async def delete_node(self, node_id: int, user_id: int | None = None) -> bool:
        ...
        # Batch-fetch all backlink source nodes and compute updated content once
        # per source, then issue a single bulk update.
        updated_nodes = set()
        updates: list[tuple[int, str]] = []

        source_ids = list({link.source_node_id for link in backlinks})
        source_nodes = {
            n.id: n
            for n in (await self._node_repo.get_by_ids(source_ids) if source_ids else [])
            if n.id is not None
        }
        ...
        for source_id, source_node in source_nodes.items():
            ...
            if updated_content != source_node.name:
                updates.append((source_id, updated_content))
                updated_nodes.add(source_id)

        if updates:
            await self._node_repo.update_names_batch(updates, user_id or self._user_id)
            logger.debug("[DELETE] Bulk-updated %s source nodes", len(updates))
    ...
    async def permanently_delete_node(self, node_id: int) -> bool:
        ...
        if updates:
            await self._node_repo.update_names_batch(updates, self._user_id)
            logger.debug("[PERM_DELETE] Bulk-updated %s source nodes", len(updates))
    ...
    async def _replace_inline_class_references(self, node: Node, already_updated: set) -> None:
        ...
        inline_refs = await self._link_service.get_inline_class_references(node.id)

        pending_source_ids = [
            ref.source_id for ref in inline_refs if ref.source_id not in already_updated
        ]
        source_nodes = {
            n.id: n
            for n in (await self._node_repo.get_by_ids(pending_source_ids) if pending_source_ids else [])
            if n.id is not None
        }
    ...
    async def find_user_ids_by_page_node_uuids(self, node_uuids: list[str]) -> dict[str, int | None]:
        """Find user IDs for multiple profile page node UUIDs in one batch."""
        if self._user_repo is None:
            return dict.fromkeys(node_uuids, None)
        return await self._user_repo.get_user_ids_by_page_node_uuids(node_uuids)
    ...
    async def get_node_suggestions(
        ...
        if class_filter_ids:
            ...
            if extension_service is not None:
                ...
                try:
                    extended_map = await extension_service.get_all_extended_classes_batch(
                        class_filter_ids
                    )
                    for class_id in class_filter_ids:
                        expanded.add(class_id)
                        expanded.update(extended_map.get(class_id, []))
                except (ValueError, LookupError, RecursionError):
                    expanded.update(class_filter_ids)
    ...
    async def clear_scratchpad(self, user_id: int) -> dict[str, Any]:
        ...
        """
        from app.domain.entities.constants import SYSTEM_PAGE_UUIDS
    ...
    async def load_node_children(
        ...
        if descendant_ids:
            backlink_counts = await self._link_service.get_backlink_counts(
                descendant_ids
            )
        ...
        if all_source_ids:
            target_ids = await self._link_service.get_text_link_targets_batch(
                all_source_ids
            )
            if target_ids:
    ...
    async def add_class_with_side_effects(self, node_id: int, class_node_id: int) -> Node:
        """Add a class atomically; create query-class main_content view if needed."""
        node = await self._node_repo.get_by_id(node_id)
        if node is None:
            raise NodeNotFoundError(f"Node {node_id} not found")

        async with get_transaction():
            added = await self.add_class(node_id, class_node_id)
            if added and self._view_repo is not None and self._workspace_id:
                class_node = await self._node_repo.get_by_id(class_node_id)
                if class_node and class_node.uuid == SYSTEM_CLASS_UUIDS["query"]:
                    existing_views = await self._view_repo.list_by_node(node_id, view_type="main_content")
    ...
    async def remove_class_with_side_effects(self, node_id: int, class_node_id: int) -> Node:
        """Remove a class atomically; delete query-class main_content views if needed."""
        node = await self._node_repo.get_by_id(node_id)
        if node is None:
            raise NodeNotFoundError(f"Node {node_id} not found")
        ...
        async with get_transaction():
            ...
            if (
                class_node
                and class_node.uuid == SYSTEM_CLASS_UUIDS["query"]
                and self._view_repo is not None
                and self._workspace_id
            ):
                existing_views = await self._view_repo.list_by_node(node_id, view_type="main_content")
                for view in existing_views:
                    await self._view_repo.delete(view.id)

            await self.remove_class(node_id, class_node_id)
    ...
    async def add_tag_link_atomic(self, node_id: int, target_node_id: int) -> Node:
        """Add a tag link atomically."""
        async with get_transaction():
            await self.add_tag_link(node_id, target_node_id)
            node = await self._node_repo.get_by_id(node_id)
            if node is None:
                raise NodeNotFoundError(f"Node {node_id} not found")
            return node

    async def remove_tag_link_atomic(self, node_id: int, target_node_id: int) -> Node:
        """Remove a tag link atomically; raises NodeNotFoundError if link did not exist."""
        async with get_transaction():
            removed = await self.remove_tag_link(node_id, target_node_id)
            if not removed:
                raise NodeNotFoundError(f"Tag link {target_node_id} not found on node {node_id}")
            node = await self._node_repo.get_by_id(node_id)
            if node is None:
                raise NodeNotFoundError(f"Node {node_id} not found")
            return node
    ...
    async def set_property_value_atomic(self, node_id: int, property_id: int, value: Any) -> Node:
        ...
        async with get_transaction():
            ...
            if prop.type in SCALAR_TYPES:
                ...
                await self._property_repo.set_relation_value(node_id, property_id, value)
            elif prop.type == PropertyType.SELECTION:
                await self._property_repo.set_selection_value(node_id, property_id, value)
    ...
    async def remove_property_atomic(self, node_id: int, property_id: int) -> Node:
        """Remove a property assignment atomically."""
        async with get_transaction():
            await self._property_repo.remove_property_from_node(node_id, property_id)
            node = await self._node_repo.get_by_id(node_id)
            if node is None:
                raise NodeNotFoundError(f"Node {node_id} not found")
            return node

    async def add_alias_atomic(self, node_id: int, alias_node_id: int) -> Node:
        """Add an alias atomically."""
        async with get_transaction():
            await self.add_alias(node_id, alias_node_id)
            node = await self._node_repo.get_by_id(node_id)
            if node is None:
                raise NodeNotFoundError(f"Node {node_id} not found")
            return node

    async def remove_alias_atomic(self, node_id: int, alias_node_id: int) -> Node:
        """Remove an alias atomically; raises NodeNotFoundError if alias did not exist."""
        async with get_transaction():
            removed = await self.remove_alias(node_id, alias_node_id)
            if not removed:
                raise NodeNotFoundError(f"Alias {alias_node_id} not found on node {node_id}")
            node = await self._node_repo.get_by_id(node_id)
            if node is None:
                raise NodeNotFoundError(f"Node {node_id} not found")
            return node
    ...
    async def merge_pages(
        ...
        backlink_source_ids = await self._link_service.get_backlink_source_ids(source_id)
        backlink_source_ids = [sid for sid in backlink_source_ids if sid != target_id]

        if source.uuid and target.uuid:
            source_nodes = {
                n.id: n
                for n in (await self._node_repo.get_by_ids(backlink_source_ids) if backlink_source_ids else [])
                if n.id is not None
            }
            updates: list[tuple[int, str]] = []
            for bsid, source_node in source_nodes.items():
                if not source_node.name:
                    continue
                ...
                if updated != source_node.name:
                    updates.append((bsid, updated))
            if updates:
                await self._node_repo.update_names_batch(updates, self._user_id)

        # Step 5: Redirect structural backlinks in node_link table
        await self._link_service.redirect_link_targets(source_id, target_id)
        ...
        await self._link_service.delete_source_links(source_id)
