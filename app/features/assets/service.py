...
def _validate_asset_uuid(asset_uuid: str) -> uuid_module.UUID:
    ...
    """
    try:
        return uuid_module.UUID(asset_uuid)
...
class AssetFileService:
    ...
    def _validate_asset_uuid(self, asset_uuid: str) -> uuid_module.UUID:
        """Validate that ``asset_uuid`` is a well-formed UUID."""
        return _validate_asset_uuid(asset_uuid)

    def _resolve_asset_path(self, asset_uuid: str) -> Path:
        """Return the resolved path for ``asset_uuid`` inside the assets directory.

        Validates the UUID format and ensures the resolved path stays within the
        workspace assets directory, blocking path-traversal attempts even if the
        caller bypasses FastAPI's path-parameter routing.

        Raises:
            AssetPermissionError: If the UUID is malformed or the path escapes
                ``assets_dir``.
        """
        self._validate_asset_uuid(asset_uuid)
        target = (self.assets_dir / asset_uuid).resolve()
        if not target.is_relative_to(self._assets_dir_resolved):
            raise AssetPermissionError(f"Asset path escapes assets directory: {asset_uuid}")
        return target

    def get_asset_folder(self, asset_uuid: str) -> Path:
        """Get the folder path for an asset."""
        return self._resolve_asset_path(asset_uuid)
