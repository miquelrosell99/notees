"""Tracked migration: Convert raw [[uuid]] text references to broken_link AST nodes.

Scans all active nodes' AST content for text nodes containing [[uuid]] or
[label]([[uuid]]) patterns where the target UUID does NOT exist, and replaces
them with proper broken_link AST nodes.
"""
...
def _transform_text_node(text_value: str, missing_uuids: set[str]) -> list[dict]:
    parts: list[dict] = []
    last_end = 0
    ...
    for match in UUID_PATTERN.finditer(text_value):
        ...
        broken_link: dict = {"type": "broken_link", "link_id": link_id}
        if label:
...
def _walk_and_transform(nodes: list, missing_uuids: set[str]) -> tuple[list, int]:
    converted = 0
    new_nodes: list = []
    ...
    for node in nodes:
        ...
        if node.get("type") == "text":
            ...
            if "[[" in text_val and UUID_PATTERN.search(text_val):
                replacement = _transform_text_node(text_val, missing_uuids)
            ...
            child_nodes, child_converted = _walk_and_transform(node["children"], missing_uuids)
...
async def run(conn: asyncpg.Connection) -> None:
    ...
    for row in rows:
        ...
        def _collect_uuids(nodes: list) -> None:
            for n in nodes:
                ...
                if n.get("type") == "text":
                    ...
                    if "[[" in text:
                        for m in UUID_PATTERN.finditer(text):
                            u = (m.group("uuid_labeled") or m.group("uuid_bare")).lower()
                            all_referenced_uuids.add(u)
                if "children" in n:
                    _collect_uuids(n["children"])
    ...
    for row in rows:
        ...
        try:
            new_ast, converted = _walk_and_transform(ast, missing_uuids)
            if converted > 0:
