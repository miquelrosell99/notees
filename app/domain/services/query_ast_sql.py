...
class QueryASTToSQL:
    ...
    def __init__(self, workspace_id: int, current_node_uuid: str | None = None):
        """
    ...
    def _generate_scope_sql(self, scope: ScopeNode) -> str | None:
        ...
        if scope.scope_type == ScopeType.ENTIRE_WORKSPACE:
            ...
            if scope.include_descendants:
                ...
                return "(n.page_id = (SELECT id FROM node WHERE uuid = %(current_uuid)s::uuid AND workspace_id = %(workspace_id)s))"
            else:
                # Only the current page itself
                return "(n.uuid = %(current_uuid)s::uuid)"
    ...
    def _generate_reference_path_condition(self, condition: ReferencePathCondition) -> str | None:
        ...
        if condition.target_uuids:
            ...
            target_subquery = (
                f"(SELECT id FROM node WHERE uuid = ANY(%({param_name})s) AND workspace_id = %(workspace_id)s)"
            )
        ...
        if condition.nested_group and condition.nested_group.children:
            ...
            if hasattr(first_block, "value"):
                ...
                target_subquery = (
                    f"(SELECT id FROM node WHERE uuid = %({param_name})s AND workspace_id = %(workspace_id)s)"
                )
                target_subquery2 = (
                    f"(SELECT id FROM node WHERE uuid = %({param_name2})s AND workspace_id = %(workspace_id)s)"
                )
                target_subquery3 = (
                    f"(SELECT id FROM node WHERE uuid = %({param_name3})s AND workspace_id = %(workspace_id)s)"
                )
...
def generate_sql_from_ast(
    ast: QueryAST, workspace_id: int, current_node_uuid: str | None = None
