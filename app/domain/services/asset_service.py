...
class AssetService:
    ...
    def find_source_file(self, asset_uuid: str) -> Optional[Path]:
        """
        ...
        if len(source_files) == 0:
            raise AssetInvariantViolation(
                f"Asset folder has no source file: {asset_uuid}"
            )
    ...
    async def create_asset(
        ...
        try:
            ...
            raise AssetError(f"Failed to create asset: {e}")
    ...
    async def replace_asset(
        ...
        try:
            ...
            if mime_category_changed:
                logger.warning(
                    f"Asset {asset_uuid} MIME category changed: {old_extension} → {new_extension}"
                )
            ...
            raise AssetError(f"Failed to replace asset: {e}")
    ...
    def _mime_category_changed(self, old_ext: str, new_ext: str) -> bool:
        ...
        from app.routers.assets import get_asset_category, get_extension_from_content_type, ALLOWED_CONTENT_TYPES
    ...
    def verify_asset(self, asset_uuid: str) -> Tuple[bool, Optional[str], Optional[str]]:
        """
