...
def validate_node_name(name: str) -> None:
    ...
    if len(name) > MAX_NAME_LENGTH:
        raise ValidationError(f"Node name is too long (max {MAX_NAME_LENGTH} characters, got {len(name)})")
    ...
    if CONTROL_CHAR_PATTERN.search(name):
        raise ValidationError("Node name contains invalid control characters")
...
def validate_icon(icon: str | None) -> None:
    ...
    if len(icon) > MAX_ICON_LENGTH:
        raise ValidationError(
            f"Icon string exceeds maximum length of {MAX_ICON_LENGTH} characters (got {len(icon)} characters)"
        )

    # Check for control characters
    if CONTROL_CHAR_PATTERN.search(icon):
        raise ValidationError("Icon string contains invalid control characters")
...
def validate_color(color: str | None) -> None:
    ...
    if len(color) > MAX_COLOR_LENGTH:
        raise ValidationError(
            f"Color string exceeds maximum length of {MAX_COLOR_LENGTH} characters (got {len(color)} characters)"
        )

    # Basic color format validation
    if not VALID_COLOR_PATTERN.match(color.lower()):
        raise ValidationError(
            f"Invalid color format: {color}. Expected hex (#RRGGBB), rgb(), rgba(), hsl(), hsla(), or named color"
        )


def validate_node_create(name: str, icon: str | None = None, color: str | None = None) -> None:
...
def validate_node_update(name: str | None = None, icon: str | None = None, color: str | None = None) -> None:
