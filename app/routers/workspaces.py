...
Handles workspace creation, switching, import/export.
...
async def export_workspace(name: str, user: User = Depends(get_current_user)):
    """Export a workspace for download."""
    try:
        export_path = await db.export_workspace(user.id, name)
        return FileResponse(
            export_path,
...
async def import_workspace(
    ...
    try:
        workspace = await db.import_workspace(user.id, tmp_path, name)
        return workspace
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    finally:
