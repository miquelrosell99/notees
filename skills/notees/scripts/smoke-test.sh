...
# Some projects intentionally use only AGENTS.md as the root shell and omit
# Cursor/Claude/Codex/Gemini registration entries. When routing.yaml sets
# `no_editor_entries: true`, skip those absence checks instead of failing.
NO_EDITOR_ENTRIES=0
if [[ -f "$ROUTING_YAML" ]] && grep -qE '^no_editor_entries:\s*true' "$ROUTING_YAML"; then
  NO_EDITOR_ENTRIES=1
fi
...
if sub_runs "1b"; then
  ...
  elif [[ "$NO_EDITOR_ENTRIES" == "1" ]]; then
    pass "Cursor entry omitted (no_editor_entries: true)"
...
if sub_runs "1c"; then
  for shell in AGENTS.md CLAUDE.md CODEX.md GEMINI.md; do
    if [[ ! -f "$shell" ]]; then
      if [[ "$NO_EDITOR_ENTRIES" == "1" && "$shell" != "AGENTS.md" ]]; then
        pass "$shell omitted (no_editor_entries: true)"
      elif [[ "$TWO_ROOT" == "1" ]]; then
  ...
  if [[ "$MDC_COUNT" -gt 0 ]]; then
    ...
    pass ".cursor/rules/ omitted (no_editor_entries: true)"
  elif [[ "$TWO_ROOT" == "1" ]]; then
