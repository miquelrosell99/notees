...
**Date:** 2026-07-22  
**Branch:** `main` (re-migration from pre-ideal backup complete; verification pending)  
...
The gap analysis (`notees-migration-gap-analysis.md`) confirms this is a **fundamental rewrite**, not an incremental migration. The current Notees app has mutable PostgreSQL rows as the source of truth; the ideal architecture has an immutable operation log with client-side SQLite derived state.
...
| **Favorites** | Stored per-user in `setting_user` as a JSON array of legacy integer node IDs. | Recovered during 2026-07-22 re-migration from `pre-ideal-migration-20260717-230311.sql`; 4 `user.favorite.add` operations emitted into the relay log for workspace `Notas`. | **Yes** — synced, per-workspace, ordered. |
...
**Status:** Complete. Detailed plan and sub-task results in `phase4-frontend-cutover.md`.
...
**Status:** Complete. Detailed plan and sub-task results in `phase5-relay-hardening.md`.
...
**Status:** Complete. Detailed plan: `phase6-cleanup-deprecation.md`.
...
**Status:** In progress. Detailed plan: `notees-phase7-plus-plan.md`.
...
## Update 2026-07-21: Stale Snapshot Schema Fix + NaN Progress

**Status:** Root cause for missing favorites / empty All Pages identified and fixed.

**What was wrong:**
The 59 MB relay snapshot stored on the server had been generated by an older derived-state schema. It was missing the `active` column on `node` and the `user_favorite` table. Because the snapshot's SQLite `user_version` was already high, `createSchema()` migrations were skipped after restore. As a result:
- The `all_pages` query (`n.active = 1`) matched almost nothing, so the list appeared empty except for the daily note.
- Favorites had no table to live in after the restore, so the sidebar favorites list stayed empty.
- Force re-sync restored the same stale snapshot, reproduced the bad state, and showed `NaN%` when catch-up returned zero new operations.

**What changed:**
- Bumped `CURRENT_DERIVED_STATE_VERSION` to `2` in `frontend/src/core/store.ts`. Any client whose local derived state predates the current schema now performs a hard rebuild: derived tables are cleared and the full operation log is replayed from the server instead of trusting a stale snapshot.
- Deleted the stale server snapshot rows for workspace `3b30e070-039b-47bc-ad0d-2440a2f173c5` so no client can restore them.
- Bumped the workspace `restore_epoch` to `3` to guarantee clients treat the server as restored and skip pushing potentially stale local operations back upstream.
- Guarded sync-progress math in `frontend/src/App.tsx` and `frontend/src/components/ui/SyncProgress.tsx` so `0 / 0` renders as `0%` instead of `NaN%`.

**What the user should do now:**
- Hard-reload `http://atlas:5173`.
- Expect a full rebuild progress overlay while ~3,000 operations are replayed. This may take a minute for the main workspace.
- After sync completes, the **Favorites** list and **All Pages** view should be populated correctly.

**Next:**
- Add a command-palette action that explicitly discards local IndexedDB/OPFS state and checks out fresh from the server (a stronger "reset local state" than force re-sync).
- Harden snapshot upload/download and add tests so future schema changes do not leave stale snapshots on the server.

---

## Update 2026-07-21: Discard Local State Command

**Status:** Added a command-palette action that wipes the local workspace database and rebuilds from the server.

**What changed:**
- Added `RESET_LOCAL_STATE` command ID (`sync.resetLocalState`).
- Added `resetWorkspaceStore(workspaceId)` in `frontend/src/core/adapters/workspaceStoreAdapter.ts`:
  - stops the workspace sync engine,
  - removes the in-memory registry entry,
  - removes the `UndoManager` instance,
  - deletes the persisted IndexedDB workspace database.
- Added `workspaceResetNonce` / `bumpWorkspaceResetNonce()` to `syncStatusStore` so `App.tsx` can re-run the workspace initializer after a reset.
- Wired the new command in `CommandRegistrations.tsx` with a `ConfirmationModal` explaining that unsynced offline changes will be lost.
- The action appears in the command palette as **Discard local state and check out from server**.

**What the user should do now:**
- Hard-reload `http://atlas:5173`.
- If the workspace still looks wrong, open the command palette and run **Discard local state and check out from server**. Confirm, and the workspace will rebuild from the server operation log.

---

## Update 2026-07-22: Fresh Relay Snapshot Generated

**Status:** Created a new server-side relay snapshot for the main workspace so clients can open quickly.

**What changed:**
- Discovered the main workspace had **115,714 relay envelopes**, which is why initial sync was stuck at `0%` for a long time.
- Ran `scripts/admin_create_snapshots.py --workspace-id 3b30e070-039b-47bc-ad0d-2440a2f173c5 --force` to replay the operation log server-side and create a fresh derived-state snapshot.
- The new snapshot contains 5,208 pages and 4 favorites. Its SQLite `user_version` is `0`, so the frontend's `createSchema()` migrations will run after restore and add the `active` column.
- New clients (and existing clients after the `restore_epoch` bump) will restore the snapshot and replay only envelopes newer than the snapshot HLC instead of all 115k operations.

**What the user should do now:**
- Hard-reload `http://atlas:5173`.
- Initial sync should now restore the snapshot and finish quickly.
- Favorites and **All Pages** should be populated.

---

## Update 2026-07-22: Snapshot Restore Notification Fix

**Status:** Fixed a listener-notification bug that left the UI empty after a fast snapshot restore.

**What was wrong:**
`WorkspaceStore.restoreSnapshot()` replaced the underlying sql.js database with the restored snapshot, but it never notified listeners. Hooks such as `useFavorites` and `useQueryAst` subscribed before the restore and therefore kept showing the pre-restore (empty) state. The UI appeared to load instantly but showed only the daily note and an empty favorites list.

**What changed:**
- Added a private `emitAll()` method to `WorkspaceStore`.
- Called `emitAll()` after `restoreSnapshot()`, `restoreLatestSnapshot()`, and `resetDerivedState()` so collection/list hooks re-render with the new data.

**What the user should do now:**
- Hard-reload `http://atlas:5173`.
- The workspace should restore the snapshot and the UI should update to show favorites and the full **All Pages** list.

---

## Update 2026-07-22: Actor-Aware Workspace Store Re-creation

**Status:** Fixed a store lifecycle bug that caused anonymous state to be reused after login.

**What was wrong:**
`getOrCreateWorkspaceStore()` keyed the in-memory registry only by workspace ID. When the app started before auth was ready, it created a store with `actorId = 'anonymous'`. After the user logged in, the React effect re-ran with the real user UUID, but the registry returned the existing anonymous store. Favorites (which are per-actor) were queried with `actor_id = 'anonymous'`, so the sidebar stayed empty. Other actor-scoped state was also wrong.

**What changed:**
- Added `getActorId()` to `WorkspaceStore`.
- Updated `getOrCreateWorkspaceStore()` to detect an actor mismatch, stop the old sync engine, delete the persisted local database, remove the registry entry, and create a fresh store for the new actor.
- This ensures a logged-in user always gets a store with their own actor ID and fresh data from the server snapshot.

**What the user should do now:**
- Hard-reload `http://atlas:5173`.
- Log in if needed.
- The workspace should restore the snapshot under the correct user actor, and **Favorites** should now appear.
