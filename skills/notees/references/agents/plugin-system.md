...
`registerNodeAction` contributes an item to the node context menus: the
page/block menu (`NodeContextMenu`), the inline node/class link and pill menu
(`NodeLinkContextMenu`), and — when targeted — the archived view menu
(`ArchivedNodeContextMenu`). The trash menu is deliberately closed to
contributions. Core menu items and contributed actions share a single
composition layer: each menu section renders in a canonical order with a
separator between non-empty sections, and the destructive section always
renders last.
...
| `menus` | Target menus: `'node'` (page/block), `'link'` (inline links/pills), `'archived'` (default `['node', 'link']`). |
...
| `visible(context)` | Optional predicate; receives `{ menu, nodeUuid, node, close }` (`node` may be `null` for unresolved link targets). |
