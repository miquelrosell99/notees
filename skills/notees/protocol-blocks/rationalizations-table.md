...
| "It crashed — I'll just rerun the migration from the start" | Re-running amplifies pollution. A half-completed Phase 5 leaves `notees` stubs that a Phase 3 rerun cannot see; Phase 8 then passes on a broken tree. Either **detect-and-resume** via `.migration-state`, or `rm -rf` the skill tree first. No in-between |
