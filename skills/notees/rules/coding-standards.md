...
- **Display helpers that cannot resolve a `node_link` must fall back to the link label or a non-UUID placeholder (`Link`), never the target UUID or "…" placeholder.** `deriveName` and `nodeNameToText` are used by many surfaces (breadcrumbs, sidebars, search, command palette, cards). Returning "…" makes link-only nodes indistinguishable; returning the raw target UUID leaks internal identifiers to users. Use `Link` (or the link's custom label) so the surface stays usable without exposing UUIDs.
