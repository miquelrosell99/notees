...
- **Display helpers that cannot resolve a `node_link` fall back to the link label first, then the target UUID from `link_id`, never an "…" placeholder.** `deriveName` and `nodeNameToText` are used by many surfaces (breadcrumbs, sidebars, search, command palette, cards). Returning "…" makes link-only nodes indistinguishable; showing the target UUID keeps the surface identifiable when nothing better exists. (Updated 2026-08: previously the rule mandated a non-UUID `Link` placeholder; the owner decided the target UUID is the preferred last-resort fallback — see commit fe16e8b8.)
